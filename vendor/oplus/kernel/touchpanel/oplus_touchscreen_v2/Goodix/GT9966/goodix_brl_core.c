...
static int  goodix_pen_control_palm_inpen(struct chip_data_brl *chip_info, int ctl_cmd);

static void start_time(struct chip_data_brl *chip_info, choice_one_hrtimer choice_one_hrtimer) {
...
{
	TPD_INFO("GT:%s find vibrator cmd and set cmd:%d\n", __func__, BIT_CHK(ctl_cmd, PEN_CTL_VIBRATOR_ENABLE));

	if (goodix_send_cmd_simple(chip_info, PEN_CMD_VIBRATOR, !(BIT_CHK(ctl_cmd, PEN_CTL_VIBRATOR_ENABLE))) < 0) {
...
static void goodix_clear_status(struct chip_data_brl *chip_info)
{
...
static void goodix_get_pen_points(void *chip_data, struct pen_info *pen_info)
{
	...
	if (chip_info->touch_state == TOUCH_DOWN) {
		TPD_DEBUG("GT:%s:detect touch down, must release all touch!!\n", __func__);
		chip_info->touch_state = TOUCH_UP;
				   ...
				   void *chip_data, debug_type debug_type)
{
	...
	ret = gt_transfer_check_data(chip_info, ic_info,
	            kernel_buf, clear_state, debug_type);
	if (ret < 0) {
		TPD_INFO("GT:data not ready, quit!\n");
		goto read_data_exit;
	}
...
static int pencil_parse_data(struct chip_data_brl *chip_info, char *buf, int cnt)
{
	...
	if (kstrtou16(tmp, PENCIL_STR_TO_HEX, &uch_value)) {
		TPD_INFO("GT:%s: kstrtoint error\n", __func__);
		goto ERR;
	}

	chip_info->checkdata[cnt] = uch_value;
	TPD_DEBUG("GT:%s:tmp[%s]checkdata[%d]hex[0x%4x]dec[%hu]\n", __func__,
...
{
	if (!strncmp(check_buf, PENCIL_DATA_ADDR2, PENCIL_DATA_MATCH))
		if (pencil_data_handle(chip_info, strcpy(check_buf, check_buf + PENCIL_DATA_MATCH), P_ADDR2) < 0)
			TPD_INFO("GT:%s:operation failed!!\n", __func__);
}
...
static ssize_t proc_pencil_healthinfo_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ts->is_suspended) {
		TPD_INFO("GT:%s:now suspend, not to set", __func__);
		goto OUT;
	}

	if (!chip_info->pen_enable) {
		TPD_DEBUG("GT:%s:now pen disable, not set", __func__);
		goto OUT;
	}

	if (count != PENCIL_CHECK_COUNT) {
		TPD_DEBUG("GT:%s count not %d\n", __func__, count);
		goto OUT;
	}

	if (tp_copy_from_user(get_buf, sizeof(get_buf), buffer, count, MAX_PENCIL_DATA)) {
		TPD_INFO("GT:%s: read proc input error.\n", __func__);
		goto OUT;
	}

	TPD_DEBUG("GT:%s:pen register->char:[%s], count:%d\n", __func__, get_buf, count);
...
static void init_gt9966_debug_node(struct chip_data_brl *chip_info)
{
	...
	if (chip_info->pen_support == true) {
		...
		if (prEntry_tmp == NULL) {
			...
			TPD_INFO("GT:%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
		} else {
			chip_info->pen_healthinfo_node = true;
			TPD_INFO("GT:%s: create proc %s pass, %d\n", __func__, PENCIL_DATA_PATH, __LINE__);
			chip_info->pen_err_coding_cnt = 0;
			chip_info->pen_err_adc_cnt    = 0;
			chip_info->pen_err_base_cnt   = 0;
		}
...
static int goodix_gt9966_ts_probe(struct spi_device *spi)
{
	...
	init_gt9966_debug_node(chip_info);
