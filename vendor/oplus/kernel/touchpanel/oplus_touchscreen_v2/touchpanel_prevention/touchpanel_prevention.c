...
static bool research_point_landed_ver_v4(struct kernel_grip_info *grip_info, uint32_t research_pos_bits, struct point_info *points, int id)
{
	...
	bool ret = false;
	uint8_t pos = grip_info->points_pos[id];
	uint16_t long_hold_debounce_ms = grip_info->large_long_debounce_ms;
	struct point_info cur_p = points[id];

	if (cur_p.x > grip_info->large_long_x1_width && (grip_info->max_x - cur_p.x) > grip_info->large_long_x1_width) {
		return ret;
	}
...
static bool corner_exit_matched(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	if ((STATUS_CENTER_DOWN != grip_info->points_center_down[index]) && (TYPE_CORNER_LARGE_SIZE != grip_info->large_finger_status[index]) && (TYPE_LONG_FINGER_HOLD != grip_info->large_finger_status[index])) {
		if (max_distance > exit_dis) {
			...
			grip_info->grip_moni_data.corner_exit_dis_match_times++;
		}
	}
	...
	if ((cur_p.rx_er * cur_p.rx_press <= xfsr_coupling_thd) && (cur_p.tx_er * cur_p.tx_press <= yfsr_coupling_thd)) {
		...
		grip_info->exit_match_times[index]++;
		grip_info->grip_moni_data.corner_press_exit_match_times++;
	}
...
static void finger_max_rx_matched(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	if (cur_p.rx_press > grip_info->max_rx_rejec_thd) {
		...
		if (grip_info->max_rx_matched_cnt[id] > 1) {
			...
			if (grip_info->max_rx_stable_time[id] > grip_info->max_rx_stable_time_thd) {
...
static bool dynamic_finger_hold_matched(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	if (cur_p.x > grip_info->dynamic_finger_hold_size_x && abs(grip_info->max_x - cur_p.x) > grip_info->dynamic_finger_hold_size_x) {
		return false;
	}
	...
	uint16_t *edge_narrow_witdh, uint16_t *edge_exit_dis)
{
	...
	if (grip_info->finger_hold_max_rx_matched[index] > MAX_RX_THRESHOLD) {
		*edge_exit_dis = grip_info->finger_hold_max_rx_exit_distance;
		*edge_narrow_witdh = grip_info->finger_hold_max_rx_narrow_witdh;
		...
		if (dynamic_finger_hold_matched(grip_info, points, index)) {
			*edge_exit_dis = grip_info->dynamic_finger_hold_exit_distance;
			*edge_narrow_witdh = grip_info->dynamic_finger_hold_narrow_witdh;
		} else {
		...
		*edge_exit_dis = grip_info->max_rx_exit_distance;
		*edge_narrow_witdh = grip_info->max_rx_narrow_witdh;
	} else {
		*edge_exit_dis = grip_info->edge_swipe_exit_distance;
		*edge_narrow_witdh = grip_info->edge_swipe_narrow_witdh;
	}
	...
	if (STATUS_CENTER_DOWN != grip_info->points_center_down[index]) {
		if ((POS_LONG_LEFT == pos || POS_LONG_RIGHT == pos) && (TYPE_PALM_LONG_SIZE != grip_info->large_finger_status[index]) && (TYPE_LONG_FINGER_HOLD != grip_info->large_finger_status[index])) {
			if (abs(cur_p.x - first_p.x) < edge_narrow_witdh && abs(cur_p.y - first_p.y) > edge_exit_dis) {
				...
				grip_info->grip_moni_data.exit_match_y_times++;
			}
		}
		if ((POS_SHORT_LEFT == pos || POS_SHORT_RIGHT == pos) && (TYPE_PALM_SHORT_SIZE != grip_info->large_finger_status[index])) {
			if (abs(cur_p.y - first_p.y) < edge_narrow_witdh && abs(cur_p.x - first_p.x) > edge_exit_dis) {
				...
				grip_info->grip_moni_data.exit_match_x_times++;
			}
		}
	}
...
static bool finger_hold_matched_v4(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if ((tmp_pos == pos) ||
		    ...
		    (POS_LONG_RIGHT == pos && POS_VERTICAL_RIGHT_CORNER == tmp_pos) ||
		    (POS_VERTICAL_RIGHT_CORNER == pos && POS_LONG_RIGHT == tmp_pos)) {
			if (cur_p.rx_press > grip_info->finger_hold_rx_rejec_thd) {
...
static bool finger_hold_different_size(struct kernel_grip_info *grip_info, struct point_info *points, int id)
{
	...
	if (cur_p.x > grip_info->finger_hold_differ_size_x && abs(grip_info->max_x - cur_p.x) > grip_info->finger_hold_differ_size_x) {
		return false;
	}
	...
	for (index = 0; index < TOUCH_MAX_NUM; index++) {
		...
		if (grip_info->first_point[index].x > grip_info->finger_hold_differ_size_x
			&& abs(grip_info->max_x - grip_info->first_point[index].x) > grip_info->finger_hold_differ_size_x) {
			break;
		}

		if (cur_p.x < grip_info->finger_hold_differ_size_x) {
			is_left_right_pos = 0;
		} else if (abs(grip_info->max_x - cur_p.x) < grip_info->finger_hold_differ_size_x) {
		    is_left_right_pos = 1;
		}

		if (grip_info->first_point[index].x < grip_info->finger_hold_differ_size_x) {
			is_left_right_temp_pos = 0;
		} else if (abs(grip_info->max_x - grip_info->first_point[index].x) < grip_info->finger_hold_differ_size_x) {
		    is_left_right_temp_pos = 1;
		}

		if (is_left_right_pos != is_left_right_temp_pos) {
			under_time_interval = abs(grip_info->first_point[index].time_ms - grip_info->first_point[id].time_ms) < grip_info->finger_hold_differ_size_debounce_ms;
...
static enum large_judge_status large_shape_judged_V2(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		...
		if (grip_info->is_curved_screen_v4 || grip_info->corner_eliminate_point_support) {
			if (research_point_landed(grip_info, grip_info->corner_eliminate_point_type, points, index)) {
				judge_status = JUDGE_LARGE_OK;
...
static const struct key_addr key_addr_arrays[] = {
	...
	{"long_start_coupling_thd",		(u64)&(((struct kernel_grip_info *)0)->long_start_coupling_thd)},
	{"long_stable_coupling_thd",		(u64)&(((struct kernel_grip_info *)0)->long_stable_coupling_thd)},
	{"long_detect_time_ms",			(u64)&(((struct kernel_grip_info *)0)->long_detect_time_ms)},
	{"long_hold_changed_thd",		(u64)&(((struct kernel_grip_info *)0)->long_hold_changed_thd)},
	{"long_hold_maxfsr_gap",		(u64)&(((struct kernel_grip_info *)0)->long_hold_maxfsr_gap)},
	...
	{"large_long_x1_width",	(u64)&(((struct kernel_grip_info *)0)->large_long_x1_width)},
	{"large_long_y1_width",	(u64)&(((struct kernel_grip_info *)0)->large_long_y1_width)},
	{"large_long_debounce_ms",	(u64)&(((struct kernel_grip_info *)0)->large_long_debounce_ms)},
	...
	{"finger_hold_differ_size_x", (u64)&(((struct kernel_grip_info *)0)->finger_hold_differ_size_x)},
	{"finger_hold_differ_size_debounce_ms", (u64)&(((struct kernel_grip_info *)0)->finger_hold_differ_size_debounce_ms)},
	{"finger_hold_rx_rejec_thd",		(u64)&(((struct kernel_grip_info *)0)->finger_hold_rx_rejec_thd)},
	{"finger_hold_max_rx_exit_distance", 	(u64)&(((struct kernel_grip_info *)0)->finger_hold_max_rx_exit_distance)},
	{"finger_hold_max_rx_narrow_witdh",		(u64)&(((struct kernel_grip_info *)0)->finger_hold_max_rx_narrow_witdh)},
	{"max_rx_matched_support",		(u64)&(((struct kernel_grip_info *)0)->max_rx_matched_support)},
	{"max_rx_rejec_thd",		(u64)&(((struct kernel_grip_info *)0)->max_rx_rejec_thd)},
	{"max_rx_stable_time_thd",		(u64)&(((struct kernel_grip_info *)0)->max_rx_stable_time_thd)},
	{"max_rx_exit_distance",	(u64)&(((struct kernel_grip_info *)0)->max_rx_exit_distance)},
	{"max_rx_narrow_witdh", 	(u64)&(((struct kernel_grip_info *)0)->max_rx_narrow_witdh)},
	{"dynamic_finger_hold_exit_support",		(u64)&(((struct kernel_grip_info *)0)->dynamic_finger_hold_exit_support)},
	{"dynamic_finger_hold_exit_distance",		(u64)&(((struct kernel_grip_info *)0)->dynamic_finger_hold_exit_distance)},
	{"dynamic_finger_hold_narrow_witdh",	(u64)&(((struct kernel_grip_info *)0)->dynamic_finger_hold_narrow_witdh)},
	{"dynamic_finger_hold_size_x", 	(u64)&(((struct kernel_grip_info *)0)->dynamic_finger_hold_size_x)},
};
...
static int kernel_grip_init_v4(struct kernel_grip_info *grip_info, struct device *dev)
{
	...
	if (ret) {
		...
		grip_info->large_long_x1_width = 0;
		grip_info->large_long_y1_width = 0;
		grip_info->large_long_debounce_ms = 300;
		...
		grip_info->large_long_x1_width = temp_array[3];
		grip_info->large_long_y1_width = temp_array[4];
		grip_info->large_long_debounce_ms = temp_array[5];
	...
	if (ret) {
		...
		grip_info->finger_hold_differ_size_x = 0;
		grip_info->finger_hold_differ_size_debounce_ms = 300;
		...
		grip_info->finger_hold_differ_size_x = temp_array[2];
		grip_info->finger_hold_differ_size_debounce_ms = temp_array[3];
	}
	ret = of_property_read_u32_array(dev->of_node, (char *)"prevention,finger_hold_max_rx_exit_distance", temp_array, 3);
	if (ret) {
		grip_info->finger_hold_rx_rejec_thd = 0xff;
		grip_info->finger_hold_max_rx_narrow_witdh = grip_info->edge_swipe_narrow_witdh;
		grip_info->finger_hold_max_rx_exit_distance = grip_info->edge_swipe_exit_distance;
		TPD_INFO("finger_hold_max_rx_exit_distance config using default.\n");
	} else {
		grip_info->finger_hold_rx_rejec_thd = temp_array[0];
		grip_info->finger_hold_max_rx_narrow_witdh = temp_array[1];
		grip_info->finger_hold_max_rx_exit_distance = temp_array[2];
	}
	...
	if (ret) {
		...
		grip_info->max_rx_rejec_thd = 0xff;
		grip_info->max_rx_stable_time_thd = 150;
		grip_info->max_rx_narrow_witdh = grip_info->edge_swipe_narrow_witdh;
		grip_info->max_rx_exit_distance = grip_info->edge_swipe_exit_distance;
		TPD_INFO("max_rx_exit_distance config using default.\n");
	} else {
		grip_info->max_rx_matched_support = temp_array[0];
		grip_info->max_rx_rejec_thd = temp_array[1];
		grip_info->max_rx_stable_time_thd = temp_array[2];
		grip_info->max_rx_narrow_witdh = temp_array[3];
		grip_info->max_rx_exit_distance = temp_array[4];
	}
	...
	if (ret) {
		...
		grip_info->dynamic_finger_hold_narrow_witdh = grip_info->edge_swipe_narrow_witdh;
		grip_info->dynamic_finger_hold_exit_distance = grip_info->edge_swipe_exit_distance;
		grip_info->dynamic_finger_hold_size_x = 30;
		TPD_INFO("dynamic_finger_hold_exit_distance config using default.\n");
	} else {
		grip_info->dynamic_finger_hold_exit_support = temp_array[0];
		grip_info->dynamic_finger_hold_narrow_witdh = temp_array[1];
		grip_info->dynamic_finger_hold_exit_distance = temp_array[2];
		grip_info->dynamic_finger_hold_size_x = temp_array[3];
	}
	return 0;
}
