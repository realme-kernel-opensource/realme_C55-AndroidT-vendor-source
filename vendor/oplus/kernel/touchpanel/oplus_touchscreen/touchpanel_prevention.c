...
static uint8_t large_judge_pos(struct kernel_grip_info *grip_info, int index)
{
	...
	uint16_t hor_long_top_width = grip_info->large_hor_long_top_width, hor_long_top_height = grip_info->large_hor_long_top_height;
...
static bool finger_hold_matched_v4(struct kernel_grip_info *grip_info, int id)
{
	int index = 0;
	uint8_t pos = grip_info->points_pos[id], tmp_pos = 0;
	bool under_range = false, under_time_interval = false;
...
static enum large_judge_status large_shape_judged_V2(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
    ...
    if ((POS_LONG_LEFT == pos) || (POS_LONG_RIGHT == pos)) {
		...
		if (grip_info->is_curved_screen_v4 && finger_hold_matched_v4(grip_info, index)) {
			judge_status = JUDGE_LARGE_OK;
		...
		if (grip_info->is_curved_screen_v4 && top_exit_matched_v4(grip_info, points, index)) {
			judge_status = JUDGE_LARGE_TIMEOUT;
...
static void init_para_addr(struct kernel_grip_info *grip_info)
{
	...
	key_addr_arrays[index++].addr = &grip_info->large_hor_long_top_width;
	key_addr_arrays[index++].addr = &grip_info->large_hor_long_top_height;
...
static int kernel_grip_init_v4(struct kernel_grip_info *grip_info, struct device *dev)
{
	...
	ret = of_property_read_u32_array(dev->of_node, (char *)"prevention,top_corner_config2", temp_array, 4);
	if (ret) {
		grip_info->top_matched_times_thd = 3;
		grip_info->top_matched_xfsr_thd = 80;
		grip_info->large_ver_top_exit_distance = 600;
		TPD_INFO("large long finger hold long_fingerhold_condition2 using default.\n");
	} else {
	...
	if (ret) {
		grip_info->large_hor_long_top_width = 280;
		grip_info->large_hor_long_top_height = 1000;
		TPD_INFO("large long finger hold hor_long_top_corner_config using default.\n");
	} else {
		grip_info->large_hor_long_top_width = temp_array[0];
		grip_info->large_hor_long_top_height = temp_array[1];
	}
