...
extern void oplus_update_battery_status(void);
...
static int sgm41512_input_current_limit_write(int value)
{
	...
	if (g_oplus_chip && (g_oplus_chip->mmi_chg == 0)) {
		pr_err("%s: mmi_chg, limit the input current to 100mA \n",  __func__);
...
static int sgm41512_unsuspend_charger(void)
{
	...
	if (((val_reg & REG0A_SGM41512_VBUS_GOOD_STAT_MASK) >>7) == 0 && vbus < CHARGER_VBUS_VOTLAGE_REF && !fastchg_flag) {
...
static int sgm41512_enable_charger_type_det(struct charger_device *dev, bool en) {
	...
	if (en == 0 && hiz_status && (oplus_vooc_get_fastchg_started() == false || !oplus_chg_get_wait_for_ffc_flag())) {
		pr_err("%s fastchg_started = %d waiting_for_ffc = %d\n", __func__, oplus_vooc_get_fastchg_started(), oplus_chg_get_wait_for_ffc_flag());
		schedule_delayed_work(&sgm41512_threaded_irq_work, 0);
	}  else {
		if (chip->suspended_set)
